...
const TEMPLATE_FILE_ID = 'docs_id';
const DESTINATION_FOLDER_ID = 'drive_folder_id';
const CURRENCY_SIGN = '$DH';
...
function parseFormData(values, header) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      } else if (key.toLowerCase().includes("total_avance")) {
        discount += value;
        value = toCurrency(value);
    ...
    response_data["total_commande"] = toCurrency(subtotal);
    response_data["total_rest_a_payer"] = toCurrency(subtotal - discount);
...
function createDocFromForm() {
  ...
  var filename = `${response_data["Date de la commande"]}_${response_data["Company Name"]}_${response_data["Numéro de commande"]}`;
